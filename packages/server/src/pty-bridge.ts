...
export function registerPtyBridge(app: FastifyInstance, opts: PtyBridgeOptions): PtyBridgeControl {
  ...
  function commitWithPrefix(pty: IPty): boolean {
    ...
    if (bufferValid && userBuffer.length > 0) {
      ...
      process.stderr.write(
        `[vc] commit PRIMARY: buf=${userBuffer.length} prefixLen=${pendingPrefix.length}\n`,
      );
      ...
      process.stderr.write(
        `[vc] commit FALLBACK (ctrl-a/e): bufValid=${bufferValid} bufLen=${userBuffer.length} prefixLen=${pendingPrefix.length}\n`,
      );
